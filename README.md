...
- **現在の実装状況**: ヒーロー詳細表示機能まで実装済み（サービス機能はコメントアウト済み）
...
### `3-Create-a-feature-component` から `4-Add-service` への変更点（現在コメントアウト済み）

#### 追加予定機能（コメントアウト状態）
- **サービス層の導入**: `HeroService`と`MessageService`を追加
- **データ管理の分離**: コンポーネントからサービスへデータ取得ロジックを移行
- **メッセージ機能**: アプリケーション全体でのログ・メッセージ表示機能

#### ファイル追加・変更（コメントアウト状態）
```
新規追加（現在未使用）:
+ src/app/hero.service.ts           # ヒーローデータ管理サービス（コメントアウト済み）
+ src/app/hero.service.spec.ts      # ヒーローサービステスト（コメントアウト済み）
+ src/app/message.service.ts        # メッセージ管理サービス（コメントアウト済み）
+ src/app/message.service.spec.ts   # メッセージサービステスト（コメントアウト済み）
+ src/app/messages/                 # メッセージ表示コンポーネント（コメントアウト済み）

変更（現在コメントアウト済み）:
~ src/app/app.module.ts                 # サービスとメッセージコンポーネントの登録（コメントアウト済み）
~ src/app/heroes/heroes.component.ts    # サービス使用ロジック（コメントアウト済み）
~ src/app/heroes/heroes.component.html  # メッセージ連携機能（コメントアウト済み）
```

#### 実装の進化（コメントアウト状態）
| 機能 | `3-Create-a-feature-component` | `4-Add-service`（現在無効） |
|------|--------------------------------|---------------------------|
| ヒーローリスト | ✅ 実装済み | ✅ 継続 |
| ヒーロー選択 | ✅ 実装済み | ✅ 継続 |
| 詳細表示 | ✅ 独立コンポーネント | ✅ 継続 |
| データ管理 | ❌ コンポーネント内 | 🔄 サービス層（無効化済み） |
| メッセージ機能 | ❌ なし | 🔄 専用サービス（無効化済み） |

#### 技術的改善点（現在無効）
- **関心の分離**: データアクセスロジックのサービス化
- **依存性注入**: Angularの依存性注入パターンの活用
- **Observable**: 非同期データ処理の導入
- **ログ管理**: 統一されたメッセージ・ログシステム
